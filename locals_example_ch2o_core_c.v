From ch2o_compcert Require Export ch2o_lp64 locals_example_ch2o.
...
Goal stringmap_to_list δ = [
  ("main",
   local{sintT} (
     local{sintT} (
       local{sintT} (
         ! (cast{voidT%T} (var 2 ::= # intV{sintT} 17 / # intV{sintT} 2)) ;;
         ! (cast{voidT%T} (var 1 ::= # intV{sintT} 15 / # intV{sintT} 5)) ;;
         ! (cast{voidT%T} (var 0 ::= load (var 2) / load (var 1))) ;;
   ...
   local{sintT} (
     local{sintT} (
       local{sintT} (
         ...
         ! (cast{voidT%T} (var 1 ::= # intV{sintT} 15 / # intV{sintT} 5)) ;;
         ! (cast{voidT%T} (var 0 ::= load (var 2) / load (var 1))) ;;
         ret (cast{sintT%T} (load (var 0)))))) : stmt K).
Proof.
reflexivity.
Qed.
